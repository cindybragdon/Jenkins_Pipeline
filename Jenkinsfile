                    ...
                    steps {
                        sshagent(credentials : ['minikube-dev-2-ssh']) {
                            ...
                            sh '''
                                  [ -d ~/.ssh ] || mkdir ~/.ssh && chmod 0700 ~/.ssh
                                  ssh-keyscan -t rsa,dsa ${MINIKUBE} >> ~/.ssh/known_hosts
                                  ssh ${USER_MINIKUBE}@${DEPLOY_SERVER} "minikube kubectl -- create secret docker-registry regcred --docker-server=${DEPLOY_SERVER} --docker-username=${NEXUS_DOCKER_USERNAME} --docker-password=${NEXUS_PASSWORD} --docker-email=de@deploy.com --namespace=${NAMESPACE}"
